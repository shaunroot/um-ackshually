...
const DEFAULTS = { wpUrl: 'https://www.shaunroot.net/wp-json/umack/v1', apiKey: '', enabled: false, delaySeconds: 20 };
const $ = (id) => document.getElementById(id);

api.storage.local.get(DEFAULTS).then((s) => {
  $('wpUrl').value = s.wpUrl; $('apiKey').value = s.apiKey;
  $('delaySeconds').value = s.delaySeconds; $('enabled').checked = !!s.enabled;
});
